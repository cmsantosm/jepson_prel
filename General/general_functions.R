...
get_pcoa <- function (dist, map) {
  map <- filter(map, SampleID %in% rownames(dist))
  dist <- dist[match(map$SampleID, rownames(dist)), match(map$SampleID, colnames(dist))]
  
  pcoa <- ape::pcoa(as.dist(dist))
  
  pcoa.res <- list()
  
  pcoa.res[["axes"]] <- as.data.frame(pcoa$vectors) %>%
    mutate(SampleID = rownames(.)) %>%
    inner_join(map, by = "SampleID")
  
  eigval <- round(pcoa$values$Relative_eig * 100, digits = 2)
  
  pcoa.res[["eig"]] <- data.frame( PC = 1:length(eigval), Eigval = eigval, CumEigval = cumsum(eigval))
  
  return(pcoa.res)
}

plot_pcoa <- function(pcoa.res, axis.x = 1, axis.y = 2) {
  
  ##to do: check that the requested axes exist
  
  pcoa.res$axes %>% 
    ggplot(aes(pcoa.res$axes[[axis.x]], pcoa.res$axes[[axis.y]])) +
    xlab(paste0("PCo", axis.x, " (", pcoa.res$eig$Eigval[axis.x], "%)")) +
    ylab(paste0("PCo", axis.y, " (", pcoa.res$eig$Eigval[axis.y], "%)")) 
}


get_pca <- function (mtx, map) {
  map <- filter(map, SampleID %in% rownames(mtx))
  mtx <- mtx[match(map$SampleID, rownames(mtx)), ]
  
  pca <- prcomp(mtx, scale. = T)
  ...
  pca.res[["rotations"]] <- pca$rotation %>% 
    as.data.frame() %>% 
    mutate(Variable = rownames(.)) 
  
  eigval <- round((pca$sdev^2 / sum(pca$sdev^2)) * 100, digits = 2)
  
  pca.res[["eig"]] <- data.frame( PC = 1:length(eigval), Eigval = eigval, CumEigval = cumsum(eigval))
  
  return(pca.res)
}

plot_pca <- function(pca.res, axis.x = 1, axis.y = 2) {
  
  ##to do: check that the requested axes exist
  
  pca.res$axes %>% 
    ggplot(aes(pca.res$axes[[axis.x]], pca.res$axes[[axis.y]])) +
    xlab(paste0("PC", axis.x, " (", pca.res$eig$Eigval[axis.x], "%)")) +
    ylab(paste0("PC", axis.y, " (", pca.res$eig$Eigval[axis.y], "%)")) 
}

filter_dist <- function(dist, map){
  dist <- as.matrix(dist)
  dist[row.names(dist) %in% map$SampleID, colnames(dist) %in% map$SampleID]
}
